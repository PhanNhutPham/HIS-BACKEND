...
public class KafkaProducerService {
    ...
    private static final String PATIENT_CREATE = "patient.created"; // Topic name

    public String sendPatienCreatedEvent(PatientCreate event) {
        try {
            // Gửi sự kiện và đợi kết quả (chờ cho đến khi Kafka trả lời)
            var result = kafkaTemplate.send(PATIENT_CREATE, event).get();  // Đây sẽ chờ đến khi có phản hồi từ Kafka
